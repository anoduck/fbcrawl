    ...
    if lang == "it":
        ...
        if len(newstring.split()) == 1:
            while newstring.rfind(".") != -1:
                newstring = (
                    newstring[0 : newstring.rfind(".")]
                    + newstring[newstring.rfind(".") + 1 :]
                )
            return newstring
...
def parse_date2(init_date, loader_context):
    ...
    if lang == "it":
        ...
        year, month, day = [
            int(i) for i in str(datetime.now().date()).split(sep="-")
        ]  # default is today
        ...
        # adesso, ieri, 4h, 50min
            ...
            if date[0].isalpha():
                if date[0].lower() == "ieri":
                    day = int(
                        str(datetime.now().date() - timedelta(1)).split(sep="-")[2]
                    )
                    # check that yesterday was not in another month
                    month = int(
                        str(datetime.now().date() - timedelta(1)).split(sep="-")[1]
                    )
        ...
        # l = 2
            ...
            if date[1] == "min":
                if (
                    int(str(datetime.now().time()).split(sep=":")[1]) - int(date[0])
                    >= 0
                ):
            ...
            # 4 h (oggi)
                ...
                if (
                    int(str(datetime.now().time()).split(sep=":")[0]) - int(date[0])
                    >= 0
                ):
                    return datetime(year, month, day).date()
                # 4 h (ieri)
        ...
        # l = 3
            ...
            # lunedì alle 12:34
                ...
                if delta >= 0:
                    day = int(
                        str(datetime.now().date() - timedelta(delta)).split(sep="-")[2]
                    )
                    month = int(
                        str(datetime.now().date() - timedelta(delta)).split(sep="-")[1]
                    )
                    return datetime(year, month, day).date()
    ...
    # =============================================================================
        ...
        # l = 2
            ...
            if date[1] == "min" or date[1] == "mins":
                if (
                    int(str(datetime.now().time()).split(sep=":")[1]) - int(date[0]) < 0
                    and int(str(datetime.now().time()).split(sep=":")[0]) == 0
                ):
                    day = int(
                        str(datetime.now().date() - timedelta(1)).split(sep="-")[2]
                    )
...
class FbcrawlItem(scrapy.Item):
    ...
    link = scrapy.Field(output_processor=link_cleaning)
    ...
    likes = scrapy.Field(output_processor=reactions_strip)
    ...
    newslink = scrapy.Field(output_processor=url_strip)
