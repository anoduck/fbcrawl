    ...
    def parse_page(self, response):
        ...
        if self.type == "post":
            post_id = response.xpath(
                "//div[contains(@data-ft,'top_level_post_id')]//@data-ft"
            ).get()
            if post_id is None:
            ...
            for post in response.xpath("//div[contains(@data-ft,'top_level_post_id')]"):
                ...
                yield scrapy.Request(
                    ...
                    self.parse_post,
                    priority=self.count,
                    meta={"index": 1, "post_id": many_features},
                )
    ...
    def parse_post(self, response):
        ...
        if bomb == 0:
            for reply in response.xpath(path):
                ...
                self.logger.info(
                    "{} nested comment".format(str(response.meta["index"]))
                )
                yield scrapy.Request(
                    ans,
