...
class ProfileSpider(FacebookSpider):
    ...
    def parse_page(self, response):
        ...
        if self.type == "post":
            ...
            for post in response.xpath("//div[contains(@data-ft,'top_level_post_id')]"):
                ...
                self.count -= 1
                yield scrapy.Request(
                    temp_post, self.parse_post, priority=self.count, meta={"index": 1}
                )
    ...
    def parse_post(self, response):
        ...
        if not response.xpath(path):  # prevents from exec
            ...
            for i, reply in enumerate(response.xpath(path2)):
                ...
                rep = reply.xpath(".//h3/a/@href").get()
                profile = (
                    "https://mbasic.facebook.com" + rep[: rep.find("?rc")] + "/about"
                )
                yield scrapy.Request(
                    profile,
                    callback=self.parse_profile,
    ...
    def parse_reply(self, response):
        ...
        if response.meta["flag"] == "init":
            ...
            if back:
                ...
                self.logger.info(
                    "Nested comments crawl finished, heading to proper page: {}".format(
                        response.meta["url"]
                    )
                )
                yield scrapy.Request(
            ...
            if back:
                ...
                next_reply = response.meta["url"]
                self.logger.info(
                    "Nested comments crawl finished, heading to home page: {}".format(
                        response.meta["url"]
                    )
                )
                yield scrapy.Request(
    ...
    def parse_profile(self, response):
        ...
        new.add_xpath(
            "gender", "//div[@id='basic-info']//div[@title='Gender']//div/text()"
        )
        new.add_xpath(
            "birthday", "//div[@id='basic-info']//div[@title='Birthday']//div/text()"
        )
        new.add_xpath(
            "current_city", "//div[@id='living']//div[@title='Current City']//a/text()"
        )
        new.add_xpath(
            "hometown", "//div[@id='living']//div[@title='Hometown']//a/text()"
        )
        new.add_xpath("work", "//div[@id='work']//a/text()")
        new.add_xpath("education", "//div[@id='education']//a/text()")
        new.add_xpath(
            "interested_in",
            "//div[@id='interested-in']//div[not(contains(text(),'Interested In'))]/text()",
        )
        new.add_xpath(
            "page", "//div[@id='contact-info']//div[@title='Facebook']//div/text()"
        )
        yield new.load_item()
