...
class FacebookSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "date" not in kwargs:
            ...
            self.logger.info(
                "Date attribute provided, fbcrawl will stop crawling at {}".format(
                    kwargs["date"]
                )
            )
        ...
        if "lang" not in kwargs:
            ...
            self.logger.info(
                'Language attribute recognized, using "{}" for the facebook interface'.format(
                    self.lang
                )
            )
        else:
    ...
    def parse_post(self, response):
        ...
        try:
            internal_link = response.xpath(
                "//div//a[contains(@href, '://')]/@href"
            ).get()
            internal_redirection_page = requests.get(internal_link)
            internal_redirection_page_content = internal_redirection_page.content.decode(
                "utf-8"
            ).replace(
                "\\", ""
            )
            ...
            shortened_external_link = re.findall(
                "https:[a-zA-Z0-9/.?=\n_]*", internal_redirection_page_content
            )[
                0
            ]  # find first link-like substring in content
        ...
        if not check_reactions:
            ...
            yield scrapy.Request(
                reactions, callback=self.parse_reactions, meta={"item": new}
            )
